...
async function main() {
  ...
  const products = await Promise.all([
    prisma.product.upsert({ where: { id: 1 }, update: {}, create: { name: 'Air Max 90',        description: 'Classic running shoe with visible Air unit.',   quantity: 50, price: 129.99, brandId: nike.id,    categoryId: shoes.id,       imageUrl: 'https://picsum.photos/seed/shoe1/400/300' } }),
    prisma.product.upsert({ where: { id: 2 }, update: {}, create: { name: 'Stan Smith',         description: 'Iconic leather tennis shoe, timeless style.',    quantity: 40, price: 89.99,  brandId: adidas.id,  categoryId: shoes.id,       imageUrl: 'https://picsum.photos/seed/shoe2/400/300' } }),
    prisma.product.upsert({ where: { id: 3 }, update: {}, create: { name: 'WH-1000XM5',         description: 'Industry-leading noise cancelling headphones.',  quantity: 20, price: 349.99, brandId: sony.id,    categoryId: audio.id,       imageUrl: 'https://picsum.photos/seed/head1/400/300' } }),
    prisma.product.upsert({ where: { id: 4 }, update: {}, create: { name: 'Galaxy S24',         description: 'Flagship Android smartphone with AI features.',  quantity: 30, price: 899.99, brandId: samsung.id, categoryId: phones.id,      imageUrl: 'https://picsum.photos/seed/phone1/400/300' } }),
    prisma.product.upsert({ where: { id: 5 }, update: {}, create: { name: 'iPhone 15',          description: 'Apple flagship with Dynamic Island display.',    quantity: 25, price: 999.99, brandId: apple.id,   categoryId: phones.id,      imageUrl: 'https://picsum.photos/seed/phone2/400/300' } }),
    prisma.product.upsert({ where: { id: 6 }, update: {}, create: { name: 'AirPods Pro',         description: 'Active noise cancellation with transparency.',   quantity: 60, price: 249.99, brandId: apple.id,   categoryId: audio.id,       imageUrl: 'https://picsum.photos/seed/air1/400/300' } }),
    prisma.product.upsert({ where: { id: 7 }, update: {}, create: { name: 'Dri-FIT T-Shirt',    description: 'Lightweight moisture-wicking training shirt.',   quantity: 100, price: 34.99, brandId: nike.id,    categoryId: clothing.id,    imageUrl: 'https://picsum.photos/seed/shirt1/400/300' } }),
    prisma.product.upsert({ where: { id: 8 }, update: {}, create: { name: 'Galaxy Tab S9',      description: 'Premium Android tablet with AMOLED display.',   quantity: 15, price: 699.99, brandId: samsung.id, categoryId: electronics.id, imageUrl: 'https://picsum.photos/seed/tab1/400/300' } }),
  ])
